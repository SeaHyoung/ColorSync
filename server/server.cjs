...
app.post("/api/apply-settings", (req, res) => {
  const { attributeCount, emphasisAttr, backgroundColor, keyColor, keyword } = req.body;
...
function extractHexColors(text, n = 6) {
  ...
  try {
    ...
    if (Array.isArray(maybe)) {
      const hex = maybe.map(String).map(s => s.trim()).filter(s => /^#[0-9a-fA-F]{6}$/.test(s));
...
if (USE_MOCK) {
  app.post("/api/palette", (req, res) => {
    ...
    const mock = ["#4F46E5", "#06B6D4", "#10B981", "#F59E0B", "#EF4444", "#8B5CF6"];
  ...
  app.post("/api/palette", async (req, res) => {
    ...
    if (!API_KEY) return res.status(500).json({ error: "DEEPSEEK_API_KEY missing in .env" });
...
server.on("error", (err) => console.error("🚨 server.listen error:", err?.code, err?.message));
