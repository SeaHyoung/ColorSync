...
const mixHsl = (aHex,bHex,t) => {
    ...
    let dh=B.h-A.h; if(dh>180) dh-=360; if(dh<-180) dh+=360; // hue wrap
    const h=A.h+dh*t, s=A.s+(B.s-A.s)*t, l=A.l+(B.l-A.l)*t;
    const {r,g,b:bb}=hslToRgb(h,s,l);
    return rgbToHex(r,g,bb);
};

const normalizeHex = (h) => {
    let s = String(h || "").toUpperCase().replace(/\s+/g, ""); // 공백 제거
    if (!s.startsWith("#")) s = "#" + s;
    if (!/^#[0-9A-F]{6}$/.test(s)) {
...
const textOn = (hex) => {
    const c = hexToRgb(hex); if(!c) return "#000";
...
const build21SolidTiles = (colors) => {
    ...
    for (let i = 0; i < n - 1; i++) {
        ...
        tiles.push({ hex: mixHsl(a, b, 0.50), isKey: false });
...
export default function ColorSlider({
    ...
    return (
        <div style={container} title={colors.map(normalizeHex).join(" • ")}>
            {tiles.map((t, i) => {
